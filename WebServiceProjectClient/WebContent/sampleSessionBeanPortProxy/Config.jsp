...
if (session.getAttribute("__default_endpoint__") == null) {
    session.setAttribute("__default_endpoint__", sampleSessionBeanPortProxyid._getDescriptor().getEndpoint());
}
if (submitAction != null && submitAction.equals("Update")) {
    sampleSessionBeanPortProxyid._getDescriptor().setEndpoint(request.getParameter("endpoint"));
}
else if (submitAction != null && submitAction.equals("Reset")) {
    sampleSessionBeanPortProxyid._getDescriptor().setEndpoint((String) session.getAttribute("__default_endpoint__"));
}
String endpoint = sampleSessionBeanPortProxyid._getDescriptor().getEndpoint();

boolean hasAsync = com.ibm.ccl.ws.jaxws.gstc.util.AsyncUtils.hasAsync(sampleSessionBeanPortProxyid.getClass());
